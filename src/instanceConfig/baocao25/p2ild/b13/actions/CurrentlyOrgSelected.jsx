    ...
    await pullAll(
        {
            ...
            Object.entries(groupedData).forEach(([key, list]) => {
                $("[name*=fillData]").each(function () {
                    let htmlKey = $(this).children()[2];
                    let keyTitle = (htmlKey.children[0]?.innerHTML || htmlKey.innerText).replace(/\s\s/g, ' ');
                    if (
                        keyTitle
                        ==
                        key.replace(/\s\s/g, ' ')
                    ) {
                        let less28d = filterAge(headerIndex, list, 0, 28)//Ngay tuoi
                        let less1y = filterAge(headerIndex, list, 28, 366)//Thang tuoi

                        let less5y = filterAge(headerIndex, list, 1, 5)//Nam tuoi
                        let less10y = filterAge(headerIndex, list, 5, 10)
                        let less15y = filterAge(headerIndex, list, 10, 15)
                        let less20y = filterAge(headerIndex, list, 15, 20)
                        let less30y = filterAge(headerIndex, list, 20, 30)
                        let less40y = filterAge(headerIndex, list, 30, 40)
                        let less50y = filterAge(headerIndex, list, 40, 50)
                        let less60y = filterAge(headerIndex, list, 50, 60)
                        let less70y = filterAge(headerIndex, list, 60, 70)
                        let less200y = filterAge(headerIndex, list, 70, 3000)

                        // if(groupedData[key])
                        $(this).children()[3].innerText = list.length                           //4 _Tử vong chung  _ Tổng 
                        $(this).children()[4].innerText = list.filter(e => e[headerIndex.iGioiTinh] == "F").length   //5 _Tử vong chung  _ Nữ
...
function filterAge(headerIndex, array, from, to) {
    if (to == 28) {
        return array.filter(e => {
            return (e[headerIndex.iNgayTuoi] != "" && e[headerIndex.iNgayTuoi] < 28)
                || (
                    !e[headerIndex.iNgayTuoi]
                    && !e[headerIndex.iThangTuoi]
                    && !e[headerIndex.iNamTuoi]
                )
