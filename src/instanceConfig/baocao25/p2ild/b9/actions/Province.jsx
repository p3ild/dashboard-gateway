...
export const getDataCommon = async (props) => {
    ...
    let reqPublicHealth_I = {
        ...
        orgUnit: getOrgParentIDByChildOuGroup({ orgFullArray, orgUnit: props.orgUnit, ouAggregateLevel: 3, ouChildGroup: [ORG_GROUP.TW_CSYT_KCB, ORG_GROUP.TW_YT_NGANH] }),
    ...
    let reqPublicHealth_II = [
        {
            ...
            orgUnit: getOrgParentIDByChildOuGroup({ orgFullArray, orgUnit: props.orgUnit, ouAggregateLevel: 3, ouChildGroup: [ORG_GROUP.TINH_KCB_COGIUONG_BV_DK, ORG_GROUP.TINH_KCB_COGIUONG_BV_YHCT, ORG_GROUP.TINH_KCB_COGIUONG_BV_PDL, ORG_GROUP.TINH_KCB_COGIUONG_BV_PHCN, ORG_GROUP.TINH_KCB_COGIUONG_BV_CKK, ORG_GROUP.TINH_KCB_COGIUONG_BV_DKKV, ORG_GROUP.TINH_KCB_COGIUONG_CSYT_KHAC] }),
            ...getApprovalConfig({ ...props, ds: DATASET.B9, approvalKey: 'TINH_CO_GIUONG' }),
            includeTotalRow: ["II.1", <p>Cơ sở có giường</p>],
            // sortOptions: (data, { orgUnitGroupQueryTarget }) => {
            //     if (!data) return [];
            //     let filter = orgUnitGroupQueryTarget.map(e => {
            //         return data?.filter(orgID => {
            //             return orgFullFlatMap[orgID]?.organisationUnitGroups.map(x => x.id).includes(e)
            //         })
            //     })
            //     return uniq(flatten(filter))
            // }
        },
        {
            orgUnitGroupQueryTarget: [
            ...
            orgUnit: getOrgParentIDByChildOuGroup({ orgFullArray, orgUnit: props.orgUnit, ouAggregateLevel: 3, ouChildGroup: [ORG_GROUP.TINH_CSYT_KHONGGIUONG] }),
