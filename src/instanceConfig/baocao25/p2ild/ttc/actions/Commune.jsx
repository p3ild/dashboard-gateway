...
export const getDataCommon = async (props) => {
    ...
    compareString.SPEC_CHAR = [
        ...compareString.SPEC_CHAR,
        { from: '^pk\s', to: ORG_PREFIX_REMOVE },
        { from: '^phong kham\s', to: ORG_PREFIX_REMOVE },
    ]
    ...
    let { listRow: communeHFListRow } = await listingRowByOuGroup({
        ...props,
        totalRowUsingParticularApi: true,
    ...
    communeHFListRow = communeHFListRow.map((eachRow, rowIndex) => {
        if (rowIndex !== 0) colDisable.forEach(colDisable => eachRow[colDisable] = getDisableColDataObject())
        return eachRow
    })
