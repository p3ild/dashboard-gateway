...
class NetworkUtils {
    ...
    */`)
    ...
    queryOrgByLevel = ({
        ...
        const defaultFields = `path,ancestors[id,name,code,level],code,programs,organisationUnitGroups[id,name]`
        let url = [
            this.INIT_HOST,
    ...
    findOrgByID = function ({ host = this.INIT_HOST, orgID, fields }) {
        let url = [
            `${host}/api/organisationUnits/${orgID}.json?fields=${fields ? fields + ',' : ''}:owner,displayName,id,name,level,ancestors[id,name,code,level],code,programs,children[id,level,displayName,name,code,attributeValues,organisationUnitGroups],organisationUnitGroups&paging=false`,
        ];
