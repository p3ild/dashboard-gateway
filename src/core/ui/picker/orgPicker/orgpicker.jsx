...
const deboundUpdateTree = debounce((me) => {
    ...
    // ======== ROOTKEY SLICING ========
    // If the active report defines a rootKey, slice the filtered tree to start
    // from that org's subtree. orgFullFlatMap is already populated at app init
    // so this lookup is always O(1) — no extra traversal needed.
    const { rootKey } = getPickerStateByPath('orgPickerConfig') || {};
    if (rootKey) {
        ...
        const rootNode = findNodeInTree(resultOrgTree, rootKey);
        if (rootNode) {
            resultOrgTree = [rootNode];
        }
    }
...
const generateEachOrgData = ({ orgTarget, setAllOrg }) => {
    ...
    if (!processedOrg) {
        // checkSupport failed — rescue if any descendants survive
        const survivingChildren = (orgTarget.children || [])
            .map(orgChild => generateEachOrgData({ orgTarget: orgChild, setAllOrg }))
            .filter(e => e != undefined)
            .sort((a, b) => {
        ...
        if (survivingChildren.length === 0) return undefined;
        // At least one descendant passes → parent inherits support: true
        orgTarget.support = true;
        orgTarget.children = survivingChildren;
        orgTarget['title'] = orgTarget.displayName;
        orgTarget['label'] = orgTarget.displayName;
        orgTarget['value'] = orgTarget.id + '_' + orgTarget.displayName;
        return orgTarget;
    }
...
export const OrgError = ({ orgSelected, corePicker }) => <><Tag color='red'>{orgSelected.displayName || corePicker?.orgSelected?.displayName}</Tag> không hỗ trợ xuất báo cáo này. Vui lòng chọn đơn vị khác</>
...
export default (props) => {
    ...
    let defaultValue = useMemo(() => {
        ...
        !orgSelected.support && (orgSelected = { error: OrgError({ orgSelected, corePicker }) })
