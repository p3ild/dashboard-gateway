        ...
        () => {
            ...
            (async () => {
                ...
                if (loaded && networkUtils && !me?.orgViewData) {
                    ...
                    // Build full unfiltered flat map once — never rebuilt on report changes
                    const buildFullFlatMap = (node, map) => {
                        map[node.id] = node;
                        node.children?.forEach(child => buildFullFlatMap(child, map));
                    }
                    let orgFullFlatMap = {};
                    flatten(orgViewData).forEach(e =>
                        e.organisationUnits?.forEach(root => buildFullFlatMap(root, orgFullFlatMap))
                    );
                    getPickerStateByPath('actions.setOrgFullFlatMap')(orgFullFlatMap);
